...
#if defined(NDEBUG)
# error "XElonMusk cannot be compiled without assertions."
#endif
...
const string strMessageMagic = "XElonMusk Signed Message:\n";
...
void ThreadScriptCheck() {
    RenameThread("xelonmusk-scriptch");
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJuSLCheck)
{
    AssertLockHeld(cs_main);
    // Check it again in case a previous version let a bad block in
    if (!CheckBlock(block, state, !fJuSLCheck, !fJuSLCheck))
        return false;
